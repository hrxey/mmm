...
export const subdivisionService = {
  ...
  async createSubdivision(subdivisionData: SubdivisionInsert): Promise<Subdivision> {
    ...
    const { data, error } = await supabase
      .from('subdivisions')
      .insert({ ...subdivisionData, user_id: user.id })
      .select()
      .single()
  ...
  async importSubdivisions(subdivisions: SubdivisionInsert[]): Promise<ImportResult> {
    ...
    const subdivisionsWithUser = subdivisions.map(s => ({ ...s, user_id: user.id }))
    const { data, error } = await supabase
      .from('subdivisions')
      .upsert(subdivisionsWithUser, { onConflict: 'name', ignoreDuplicates: false })
      .select('id, name')
